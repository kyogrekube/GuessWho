...
def draw_board(rows, cols, img_directory, img_list, first_generation, toggle_names_condition):
    ...
    for i in range(rows):
        for j in range(cols):
            if img_counter < len(img_list):
                ...
                board_list.append(piece)
                if toggle_names_condition == True:
                    file_name_list = img_list[img_counter].split(".")
...
cols = 10
new_board = True
current_player = 1 #this is either 1 or 2
p1_selected_face = ""
p2_selected_face = ""
p1_guessed_faces = []
p2_guessed_faces = []
...
while running:
    ...
    if restart_screen == True:
        ...
        restart_button, ask_button, guess_button, toggle_names_button = draw_backgrounds()
        draw_board(rows, cols, img_directory, img_list, first_generation, toggle_names_condition)
        first_generation = False
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if restart_button.collidepoint(event.pos):
                    ...
                    screen.blit(period1, (815, 350))
                    pygame.display.flip()
                    pygame.time.wait(1000)
                    period2 = title_font.render(".", True, red)
                    screen.blit(period1, (835, 350))
                    pygame.display.flip()
                    pygame.time.wait(1000)
                    period3 = title_font.render(".", True, red)
                    screen.blit(period3, (855, 350))
                    pygame.display.flip()
                    pygame.time.wait(1000)
